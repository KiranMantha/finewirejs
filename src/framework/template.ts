...
import { bind, resolve } from './signals.ts';
import { assertTemplate } from './validate.ts';
...
export let DEV = true;
export const setDev = (on: boolean): void => {
  DEV = on;
};
...
function flatIndex(frag: DocumentFragment): Node[] {
  ...
  for (const child of Array.from(frag.childNodes))
    for (const n of dfs(child)) out.push(n);
  return out;
}
...
function compile(strings: TemplateStringsArray): Compiled {
  ...
  const childDefs = childMarks.map(({ comment, hole }) => ({
    index: idx.get(comment)!,
    hole,
  }));
...
function setNatural(el: Element, name: string, v: unknown): void {
  if (ALWAYS_PROP.has(name)) {
    (el as any)[name] = v;
    return;
  }
  if (!ALWAYS_ATTR.has(name) && !name.includes('-') && name in el) {
    (el as any)[name] = v;
    return;
  }
...
function fillAttr(
  ...
  const compute = () => {
    ...
    return template.replace(markerRe, (_, h) =>
      String(resolve(values[Number(h)]))
    );
  };
  ...
  bind(
    holes.map((h) => values[h]),
    compute,
    (v) => {
      if (kind === 'prop') (el as any)[name] = v;
      else if (kind === 'bool')
        v ? el.setAttribute(name, '') : el.removeAttribute(name);
...
function reconcile(
  ...
  for (const n of oldNodes)
    if (!newSet.has(n) && n.parentNode === parent) parent.removeChild(n);
...
export function instantiate(result: TemplateResult): Node[] {
  ...
  for (const def of attrDefs)
    fillAttr(nodes[def.index] as Element, def, result._values);
  for (const def of childDefs)
    fillChild(nodes[def.index] as Comment, result._values[def.hole]);
