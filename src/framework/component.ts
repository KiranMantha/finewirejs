...
import {
  createRoot,
  onCleanup as coreOnCleanup,
  runWithOwner,
  getOwner,
} from './signals.ts';
import { instantiate, type TemplateResult } from './template.ts';
...
/** What defineComponent returns: call with props to create an instance. */
export type Component<P> = (props: P) => ComponentInstance;
...
export function defineComponent<P = {}>(setup: Setup<P>): Component<P> {
  return (props: P) => {
    ...
    const { value, dispose } = createRoot(() => {
      ...
      const tpl = setup(props, ctx); // ← runs ONCE, ever
    ...
    if (mounts.length) {
      queueMicrotask(() =>
        runWithOwner(owner, () => mounts.forEach((m) => m()))
      );
    }
...
const isInstance = (x: unknown): x is ComponentInstance =>
  !!x &&
  typeof x === 'object' &&
  'nodes' in (x as object) &&
  'dispose' in (x as object);
    ...
    items.forEach((item, i) => {
      ...
      if (!entry) {
        ...
        entry = runWithOwner(owner, () => {
          ...
          const { value, dispose } = createRoot(() =>
            instantiate(r as TemplateResult)
          );
          return { nodes: value, dispose };
        });
        cache.set(key, entry);
